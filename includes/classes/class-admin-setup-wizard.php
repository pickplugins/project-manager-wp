...
class qa_Admin_Setup_Wizard {
	...
	public function qa_notice_setup_wizard() {
		...
		if( get_option( 'qa_complete_setting_wizard', 'no' ) == 'no' ) {
			...
			<div id="message" class="updated">
				<p><?php _e( '<strong>Welcome to Question Answer</strong> &#8211; You did not run the setup wizard yet. You can do that now.', PM_TEXTDOMAIN ); ?></p>
				<p class="submit">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=qa-setup' ) ); ?>" class="button-primary"><?php _e( 'Run Wizard Setup', PM_TEXTDOMAIN ); ?></a> 
					<a class="button-secondary skip" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'qa-hide-notice', 'wizard' ), 'qa_hide_notices_nonce', '_qa_notice_nonce' ) ); ?>"><?php _e( 'Skip Setup', PM_TEXTDOMAIN ); ?></a>
				</p>
			</div>
			<?php
		}
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<title><?php _e( 'Question Answer &rsaquo; Setup Wizard', PM_TEXTDOMAIN ); ?></title>
	...
	public function setup_wizard_footer() {
		?>
			<?php if ( 'next_steps' === $this->step ) : ?>
				<div class="qa-return-to-dashboard"><a href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', PM_TEXTDOMAIN ); ?></a></div>
			<?php endif; ?>
			</body>
		</html>
		<?php
	...
	public function qa_setup_pages() {
		?>
		<h1><?php _e( 'Page Setup', PM_TEXTDOMAIN ); ?></h1>
		<form method="post">
			<p><?php printf( __( 'Your website needs a few essential %spages%s. The following will be created automatically (if they do not already exist):', PM_TEXTDOMAIN ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
			<table class="qa-setup-pages" cellspacing="0">
				<thead>
					<tr>
						<th class="page-name"><?php _e( 'Page Name', PM_TEXTDOMAIN ); ?></th>
						<th class="page-description"><?php _e( 'Description', PM_TEXTDOMAIN ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td class="page-name"><?php echo _x( 'Question Submission', 'Page title', PM_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will display a form where visitor can submit new question.', PM_TEXTDOMAIN ); ?></td>
					</tr>
					<tr>
						<td class="page-name"><?php echo _x( 'Question Archive', 'Page title', PM_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will show all the live questions.', PM_TEXTDOMAIN ); ?></td>
					</tr>
					<tr>
						<td class="page-name"><?php echo _x( 'My Account', 'Page title', PM_TEXTDOMAIN ); ?></td>
						<td><?php _e( 'This page will show your user account info.', PM_TEXTDOMAIN ); ?></td>
					</tr>
				</tbody>
			</table>

			<p><?php printf( __( 'Once created, these pages can be managed from your admin dashboard on the %sPages screen%s. <br> You can control which pages are shown on your website via %sAppearance > Menus%s.', PM_TEXTDOMAIN ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>', '<a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" target="_blank">', '</a>' ); ?></p>

			<p class="qa-setup-actions step">
				
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="qa-button-skip"><?php _e( 'Skip this step', PM_TEXTDOMAIN ); ?></a>
				<input type="submit" class="qa-button-next qa-input" value="<?php esc_attr_e( 'Continue', PM_TEXTDOMAIN ); ?>" name="save_step" />
	...
	public function qa_setup_question() {
		...
		?>
		<h1><?php _e( 'Question Settings', PM_TEXTDOMAIN ); ?></h1>
		<form method="post">
			<table class="form-table">

				<tr>
					<th scope="row"><label for="qa_question_item_per_page"><?php _e( 'Item per Page', PM_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_question_excerpt_length"><?php _e( 'Excerpt Length in Question Archive', PM_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_account_required_post_question"><?php _e( 'Account Required to submit Question?', PM_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_account_required_post_question" name="qa_account_required_post_question" class="qa-enhanced-select">
							<option value="yes" <?php selected( $qa_account_required_post_question, 'yes' ); ?>><?php echo __( 'Yes', PM_TEXTDOMAIN ); ?></option>
							<option value="no" <?php selected( $qa_account_required_post_question, 'no' ); ?>><?php echo __( 'No', PM_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="qa_submitted_question_status"><?php _e( 'New Submitted Question Status ?', PM_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_submitted_question_status" name="qa_submitted_question_status" class="qa-enhanced-select">
							<option value="pending" <?php selected( $qa_submitted_question_status, 'pending' ); ?>><?php echo __( 'Pending', PM_TEXTDOMAIN ); ?></option>
							<option value="draft" <?php selected( $qa_submitted_question_status, 'draft' ); ?>><?php echo __( 'Draft', PM_TEXTDOMAIN ); ?></option>
							<option value="publish" <?php selected( $qa_submitted_question_status, 'publish' ); ?>><?php echo __( 'Published', PM_TEXTDOMAIN ); ?></option>
							<option value="private" <?php selected( $qa_submitted_question_status, 'private' ); ?>><?php echo __( 'Privated', PM_TEXTDOMAIN ); ?></option>
							<option value="trash" <?php selected( $qa_submitted_question_status, 'trash' ); ?>><?php echo __( 'Trashed', PM_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public function qa_setup_answer() {
		...
		<form method="post">
			<table class="form-table">
				...
				<tr>
					<th scope="row"><label for="qa_answer_item_per_page"><?php _e( 'Item per Page', PM_TEXTDOMAIN ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="qa_account_required_post_answer"><?php _e( 'Account Required to submit Answer?', PM_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_account_required_post_answer" name="qa_account_required_post_answer" class="qa-enhanced-select">
							<option value="yes" <?php selected( $qa_account_required_post_answer, 'yes' ); ?>><?php echo __( 'Yes', PM_TEXTDOMAIN ); ?></option>
							<option value="no" <?php selected( $qa_account_required_post_answer, 'no' ); ?>><?php echo __( 'No', PM_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="qa_submitted_answer_status"><?php _e( 'New Submitted Answer Status ?', PM_TEXTDOMAIN ); ?></label></th>
					<td>
						<select id="qa_submitted_answer_status" name="qa_submitted_answer_status" class="qa-enhanced-select">
							<option value="pending" <?php selected( $qa_submitted_answer_status, 'pending' ); ?>><?php echo __( 'Pending', PM_TEXTDOMAIN ); ?></option>
							<option value="draft" <?php selected( $qa_submitted_answer_status, 'draft' ); ?>><?php echo __( 'Draft', PM_TEXTDOMAIN ); ?></option>
							<option value="publish" <?php selected( $qa_submitted_answer_status, 'publish' ); ?>><?php echo __( 'Published', PM_TEXTDOMAIN ); ?></option>
							<option value="private" <?php selected( $qa_submitted_answer_status, 'private' ); ?>><?php echo __( 'Privated', PM_TEXTDOMAIN ); ?></option>
							<option value="trash" <?php selected( $qa_submitted_answer_status, 'trash' ); ?>><?php echo __( 'Trashed', PM_TEXTDOMAIN ); ?></option>
						</select>
					</td>
				</tr>
			</table>
	...
	public function qa_setup_ready() {
		...
		<h1><?php _e( 'Your Website is Ready!', PM_TEXTDOMAIN ); ?></h1>
